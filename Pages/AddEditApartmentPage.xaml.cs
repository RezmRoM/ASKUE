...
namespace ASKUE.Pages
{
    public partial class AddEditApartmentPage : Page
    {
        ...
        public AddEditApartmentPage(AdminApartmentViewModel item)
        {
            InitializeComponent();
            ...
            else
            {
                ...
                if (residentLink != null)
                    ComboResident.SelectedValue = residentLink.id_polzovatelya;
            }
        ...
        {
            if (string.IsNullOrWhiteSpace(_current.adres) || _current.nomer_kvartiry <= 0)
            {
                MessageBox.Show("Заполните адрес и номер.");
                return;
            }

            if (_current.kvartira_id == 0)
            {
                Classes.AppContext.GetContext().Set<K_Kvartiry>().Add(_current);
            }
            ...
            try
            {
                ...
                if (ComboResident.SelectedIndex > 0)
                {
                    ...
                    if (residentLink == null)
                    {
                        Classes.AppContext.GetContext().Set<K_Prozhivayushchiye>().Add(new K_Prozhivayushchiye { id_kvartiry = _current.kvartira_id, id_polzovatelya = selectedUserId });
                    }
                    else
                    {
                        residentLink.id_polzovatelya = selectedUserId;
                    }
                }
                else if (residentLink != null)
                {
                    Classes.AppContext.GetContext().Set<K_Prozhivayushchiye>().Remove(residentLink);
                }
