...
namespace ASKUE.Windows
{
    public partial class AddEditTariffWindow : Window
    {
        ...
        public AddEditTariffWindow(K_Tarify item)
        {
                InitializeComponent();
                LoadResourceTypes();
                if (item == null) { _current = new K_Tarify { data_nachala_deystviya = DateTime.Today }; Title = "Добавление тарифа"; }
                else { _current = item; Title = "Редактирование тарифа"; }
                DataContext = _current;
            }

            private void LoadResourceTypes()
            {
                var resourceTypes = new List<ResourceType>(); // Используем класс из AddEditMeterWindow
                try
                {
            ...
            {
                if (_current.id_tipa_resursa == 0 || _current.stoimost_za_edinitsu <= 0) { MessageBox.Show("Выберите тип ресурса и укажите стоимость."); return; }
                if (_current.tarif_id == 0) Classes.AppContext.GetContext().Set<K_Tarify>().Add(_current);
